...
const state = { sceneIndex:0, scenes:[], scores:{blanc:0,bleu:0,rouge:0,or:0}, qcmScore:0, character:{}, cards:{}, qcms:{}, qcmLog:[] };
let qcmPlan = {};
...
function showScene(idx){
  ...
  (scene.options||[]).forEach((opt,k)=>{
    ...
    btn.onclick = ()=>{
      ...
      updateTopScores();
      const topicKey = qcmPlan[scene.id];
      if(topicKey){ openCarteThenQcm(topicKey); } else { showScene(idx+1); }
    };
