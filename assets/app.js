...
function initScene(){
  ...
  document.querySelectorAll('[data-choice]')?.forEach(btn=>{
    btn.addEventListener('click', (ev)=>{
      ev.preventDefault();
      const ph = btn.getAttribute('data-ph'); let hints={};
      if(ph){ ph.split(',').forEach(pair=>{ const [k,v]=pair.split(':'); if(k&&v) hints[k.trim()]=parseInt(v.trim(),10)||0; }); }
      incScores(hints);
      const next = btn.getAttribute('data-next'); if(next){ window.location.href=next; }
    });
  });
}
