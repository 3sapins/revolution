...
// Shared helpers across pages (scores, character, navigation)
const STORE = {
  character: 'revo_character',
  scores: 'revo_scores',
  qcm: 'revo_qcm'
};
...
function getCharacter(){
  try{ return ensureDefaults(JSON.parse(localStorage.getItem(STORE.character)||'{}')); }catch{return ensureDefaults({});}
}
...
function renderBadges(){ const s=getScores();
  const set=(id,v)=>{ const el=document.getElementById(id); if(el) el.textContent=v; };
  set('b-blanc', s.blanc||0); set('b-bleu', s.bleu||0); set('b-rouge', s.rouge||0); set('b-or', s.or||0); set('b-qcm', s.qcm||0);
}
...
function initStart(){
  ...
  form.addEventListener('submit', (e)=>{
    e.preventDefault(); const fd=new FormData(form); let ch=Object.fromEntries(fd.entries()); ch=ensureDefaults(ch); setCharacter(ch); window.location.href='scenes/000-introduction-1789.html';
  });
  renderBadges();
}

// Hook for scene pages
function initScene(){
  renderBadges();
  // personalize copy
  ...
  document.querySelectorAll('[data-choice]')?.forEach(btn=>{
    btn.addEventListener('click', (ev)=>{
      ...
      let hints={}; if(ph){ ph.split(',').forEach(pair=>{ const [k,v]=pair.split(':'); if(k&&v) hints[k.trim()]=parseInt(v.trim(),10)||0; }); }
      incScores(hints);
      const href = btn.getAttribute('href'); if(href){ window.location.href=href; }
    });
  });
}
...
// Hook for QCM pages
function initQcm(){ renderBadges(); const form=document.getElementById('qcm-form'); if(!form) return; form.addEventListener('submit', (e)=>{
  e.preventDefault(); const correct=parseInt(form.getAttribute('data-correct'),10)||0; const sel = form.querySelector('input[name="qcm"]:checked'); if(!sel){ alert('Choisis une réponse.'); return; }
  const ok = (parseInt(sel.value,10)===correct); incQcm(ok); const next=form.getAttribute('data-next'); if(next){ window.location.href=next; }
}); }

// Hook for summary
function initSummary(){ renderBadges(); const s=getScores();
  const set=(id,v)=>{ const el=document.getElementById(id); if(el) el.textContent=v; };
  set('sum-blanc', s.blanc||0); set('sum-bleu', s.bleu||0); set('sum-rouge', s.rouge||0); set('sum-or', s.or||0); set('sum-qcm', s.qcm||0);
}
